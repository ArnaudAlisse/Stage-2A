...
function total_demand_and_generation(network_data)
    total_pd = 0
    total_qd = 0
    total_pg = 0
    total_qg = 0
    ...
    for k in keys(network_data["gen"])
        total_pg = total_pg + network_data["gen"]["$k"]["pg"] #sum the active power generated by all generator
        total_qg = total_qg + network_data["gen"]["$k"]["qg"] #sum the reactive power generated by all generator
    end 
